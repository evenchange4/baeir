...
describe("Relation_Users_Users", (done)=>{
  ...
  describe("#record(uxxxxxxx, uzzzzzzz)", ()=>{
    let relation = {};
    before((done)=>{
      Relation_Users_Users.find({ 
        where: {
    ...
    it("should return 0 when {uid1_mention_uid2}", (done)=>{
      assert.equal(relation.uid1_mention_uid2, 0);
      done();
    });
    it("should return 2 when {uid2_mention_uid1}", (done)=>{
      assert.equal(relation.uid2_mention_uid1, 2);
      done();
    });
    it("should return 1 when {uid1_retweet_uid2}", (done)=>{
      assert.equal(relation.uid1_retweet_uid2, 1);
      done();
    });
    it("should return 2 when {uid2_retweet_uid1}", (done)=>{
      assert.equal(relation.uid2_retweet_uid1, 2);
      done();
    });
  });
